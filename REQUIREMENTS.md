...
|---|---|---|---|
| R1.1 | P0 | Regenerate the dataset ourselves with the `gameNgen-repro` ViZDoom pipeline (PPO agent, wad, and bots config are public in that repo). Render 320x240 RGB24, HUD on, crosshair off, weapon on. **Every tic is stored losslessly** (decided Sep 8): PNG bytes inside per-episode parquet files on Spiderman `/sata2` (48 KB per frame, about 240 GB for 5M tics), the same layout as the release. Encoding to latents is a separate pass over the stride-4 frames. Per decision frame record action id, health, ammo, player x/y/angle, map id, episode id, tic id. | `data/DATASET.md` lists counts; a held-out PNG sample decodes bit-exact; latent shape asserted (4, 32, 40). |
...
| R1.3 | P0 (evidence Sep 9: copy-last is 26.2 dB per-tic vs 19.3 dB at stride 4) | **Frame stride.** Store every tic, but the training and evaluation unit is one frame per agent decision (stride 4, 8.75 frames per second). Context and target are consecutive decision frames. Rationale: matches how the agent acts and makes the prediction task non-trivial. | `doom_data.py` exposes `stride`; copy-last PSNR on the val split is reported next to every model number. |
| R1.4 | P0 | **Size:** Arnold set (done Sep 9): 850 episodes x 150 s over 17 maps, 4.21M tics, 1.05M decision frames, 33.4 h, 202 GB lossless. Stiegler continuity set: 1,000 episodes, 4.95M tics, 1.24M decision frames, 219 GB. | `dataset_stats.py` output per set. |
...
| R3.4 | done Sep 9 | **Fit check at 320 tokens** on a 16 GB A4000, L = 32, velocity objective, fp32 AdamW, gradient checkpointing: DiT-XL/2 0.50 steps/s at batch 8 (11.8 GB), U-Net 0.30 steps/s at batch 4 (14.8 GB); 4-GPU torchrun 0.93 steps/s. A 60-step real-data run and the full evaluation chain also ran. | `TECHNICAL_PLAN.md` section 8 table; `RESEARCH_CONTEXT.md` Sep 9 entry. |
