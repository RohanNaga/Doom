...
**Stopping a tmux-hosted job (Sep 18 2026).** Stop it with `tmux kill-session -t <session>`, never with `pgrep -f`/`pkill -f` on a pattern from its command line: the tmux SERVER process carries the first session's whole command string in its own arguments and has the lowest pid, so a pattern match picks the server and kills every session on the machine (this took down the SD 3.5 row, the grid queue and the evaluation waiter for 35 minutes). If a pid is needed, take it from `tmux list-panes -t <session> -F '#{pane_pid}'` and its children.
...
**Live curves are mandatory (Rohan, Sep 23 2026).** Every training run streams its training loss, validation loss (overall and by quartile), gradient norm, throughput, memory and every evaluation read to Weights & Biases (project `doomdit-nexttic`) from the moment it launches, so Rohan can watch them live. `train_wm.py` does this natively and by default (`wandb_log.py`; run named after the results directory; `--no-wandb` opts out and only the gates' throwaway runs use it), and `eval_tf.py`, `rollout_eval.py` and `smoke_probe.py` append their reads with `--wandb-run <run>` to the run `<run>-eval` in the same group. The training env needs `pip install wandb` first; without it the trainer prints one `wandb:` line and trains on without curves. The sidecar `tools/wandb_tail.py --run-dir <run>`, in its own tmux session, is the fallback only for a pinned run whose certified commit predates native logging; a run without a live W&B curve is not fully launched. The API key is Rohan's: he runs `wandb login` himself, never an agent.
