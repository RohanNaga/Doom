...
# sample budget) under the identical recipe except warmup 250 (10% of the run, so the runs do not measure warmup), so the video row can be compared at matched samples. Sequential on one gpu.
...
COMMON="--context-frames 32 --num-actions 29 --global-batch 32 --lr 5e-5 --warmup 250 --clip 1.0 --steps 2500 --seed 0 --action-dropout 0.0 --require-verified-transitions --latents-dir $D/latents_arnold_aligned --split $D/split_arnold.json --val-every 500 --val-windows 1024 --ckpt-every 2500 --keep-last 1 --num-workers 4"
