...
nice -n 5 $PY $D/repo/encode_parquet.py --in-dir $IN --out-dir $OUT \
  --stride $STRIDE "${EXTRA[@]}" --batch-size $BATCH --decode-threads $THREADS \
  --device cuda:$GPU --dtype bf16 --cache-dir $D/hf/hub --decode-check 16
