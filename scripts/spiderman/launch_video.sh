...
tmux new-session -d -s train-video "cd $D/repo && TMPDIR=/sata2/data/rnagabhi/doom/tmp/tmpdir HF_HUB_OFFLINE=1 CUDA_VISIBLE_DEVICES=$GPU $([ "${GPU//[^,]/}" ] && echo "~/wanenc/bin/accelerate launch --num_processes $(( $(echo $GPU | tr -cd , | wc -c) + 1 )) --mixed_precision bf16" || echo ~/wanenc/bin/python) train_video.py --results-dir $R $COMMON $RES >> $D/logs/train_video.log 2>&1"
