...
COMMON="--context-frames $L --num-actions 29 --global-batch 8 --per-gpu-batch 1 --lr 2e-5 --warmup 500 --clip 1.0 --steps 10000 --seed 0 --action-dropout 0.0 --objective $OBJ --null-prompt $D/weights/skyreels_null_prompt.pt --latents-dir $D/latents_arnold_wan --split $D/split_arnold.json --hf-cache $D/hf/hub --val-every 500 --val-windows 512 --ckpt-every 1000 --snapshot-every 5000 --keep-last 2 --num-workers 4"
echo "$(date -Iseconds) launching video row on gpu $GPU L=$L $OBJ $RES" >> $D/logs/resumes.log
tmux new-session -d -s train-video "cd $D/repo && HF_HUB_OFFLINE=1 CUDA_VISIBLE_DEVICES=$GPU ~/wanenc/bin/python train_video.py --results-dir $R $COMMON $RES >> $D/logs/train_video.log 2>&1"
