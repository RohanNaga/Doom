#!/bin/bash
# UniDiffuser v1 row (LAION-scale transformer in the SD latent space), identical recipe to 030/031/032/033. Runs once; safe to call repeatedly. Usage: launch_unidiffuser.sh <gpu>
...
COMMON="--context-frames 32 --num-actions 29 --global-batch 32 --lr ${LR:-5e-5} --warmup 2000 --clip 1.0 --steps 90000 --seed 0 --action-dropout 0.0 --require-verified-transitions --latents-dir $D/latents_arnold_aligned --split $D/split_arnold.json --val-every 1000 --val-windows 1024 --ckpt-every 5000 --snapshot-every 5000 --keep-last 2 --num-workers 4"
