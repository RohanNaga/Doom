...
tmux new-session -d -s train-unidiffuser "cd $D/repo && TMPDIR=/sata2/data/rnagabhi/doom/tmp/tmpdir HF_HUB_OFFLINE=1 CUDA_VISIBLE_DEVICES=$GPU ~/miniconda3/envs/doom/bin/python train_wm.py --backbone unidiffuser --per-gpu-batch ${MB:-32} --warm-start thu-ml/unidiffuser-v1 --hf-cache $D/hf/hub --results-dir $R $COMMON $RES >> $D/logs/train_unidiffuser.log 2>&1"
